...
public class ListClaimController {
    ...
    @FXML
    private TextField searchField; // This is already defined in your FXML
    ...
    private void setupSearch() {
        ...
        // Lier la SortedList au TableView
        ...
        searchField.textProperty().addListener((observable, oldValue, newValue) -> {
            filteredList.setPredicate(reclamation -> {
                ...
                if (reclamation.getTitle() != null && reclamation.getTitle().toLowerCase().contains(lowerCaseFilter)) {
                    return true; // Correspond au titre
                } else if (reclamation.getEtatUser() != null && reclamation.getEtatUser().toLowerCase().contains(lowerCaseFilter)) {
                    return true; // Correspond à l'état utilisateur
                } else if (reclamation.getEtatRec() != null && reclamation.getEtatRec().toLowerCase().contains(lowerCaseFilter)) {
                    return true; // Correspond à l'état réclamation
                } else if (reclamation.getStatus() != null && reclamation.getStatus().toLowerCase().contains(lowerCaseFilter)) {
                    return true; // Correspond au statut
                } else if (reclamation.getTypeNom() != null && reclamation.getTypeNom().toLowerCase().contains(lowerCaseFilter)) {
                    return true; // Correspond au type
                }
    ...
    private void loadReclamations() {
        ...
        // Le claimsTable.setItems() est maintenant géré par setupSearch()
    }
                ...
                return new TableCell<Reclamation, Void>() {
...

                    {
                        ...
                        archiveButton.setOnAction(event -> {
                            ...
                            rec.setArchive("oui");  // ou rec.setArchive(true) selon votre modèle
                            reclamationService.modifierReclamation(rec);
                            // 2) Supprimer directement de la liste source
                            reclamationList.remove(rec);
                            // (le FilteredList/SortedList se mettra à jour automatiquement)
                        });
                    }
                    ...
                    protected void updateItem(Void item, boolean empty) {
                        ...
                        if (empty) {
                            ...
                            if (!"oui".equalsIgnoreCase(rec.isArchive())) {
