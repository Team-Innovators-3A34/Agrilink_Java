...
public class recyclingpointService implements Interface<recyclingpoint> {
    ...
    public List<recyclingpoint> pointPerPoint(User user) {
        List<recyclingpoint> recyclingpoints = new ArrayList<>();
        String req = "SELECT * FROM pointrecyclage WHERE owner_id = ?";

        try {
            PreparedStatement pst = connection.prepareStatement(req);
            pst.setInt(1, user.getId()); // Lier l'ID du propriétaire
            ...
            while (rs.next()) {
                ...
                recyclingpoints.add(new recyclingpoint(
                        id, nom, adresse, longitude, latitude, date, type, image, description,owner_id
                ));
            }
