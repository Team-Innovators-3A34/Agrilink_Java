...
public class userService implements userInterface {
            ...
            if (rs.next()) {
                ...
                if (lockUntilTimestamp != null) {
                    ...
                    if (currentTimeMillis < lockUntilMillis) {
                        long remainingMillis = lockUntilMillis - currentTimeMillis;
                        long remainingLockTimeMinutes = (long) Math.ceil(remainingMillis / (60.0 * 1000));
                        HelloApplication.error("Votre compte est verrouillé. Veuillez réessayer dans " + remainingLockTimeMinutes + " minute(s).");
                        return false;
                    } else {
    ...
    public List<User> getAllUsers() {
        ...
        Connection conn = dataBaseHelper.getInstance().getConnection();
        String query = "SELECT id, email, nom, prenom, adresse, roles, telephone, status, account_verification, is2_fa, score, image,description,longitude,latitude FROM user";
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query);
            ...
            while (rs.next()) {
                ...
                users.add(new User(id,email, nom, prenom, adresse, roles, telephone, accountVerification, image,score,description,longitude,latitude));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public boolean check2FAUser(String email) {
        Connection conn = dataBaseHelper.getInstance().getConnection();
        String query = "SELECT is2_fa FROM user WHERE email = ?"; // <-- corriger ici
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                return rs.getBoolean("is2_fa"); // <-- maintenant cette colonne existe dans le résultat
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return false; // retourne false si aucun résultat ou erreur
    }

    public void sendCode2FA(String email) {
        Connection conn = dataBaseHelper.getInstance().getConnection();

        Random random = new Random();
        String code = String.format("%04d", random.nextInt(10000));  // 4 chiffres aléatoires

        LocalDateTime expirationTime = LocalDateTime.now().plusMinutes(10); // expiration dans 10 min
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        String expirationDate = expirationTime.format(formatter);

        String query = "UPDATE user SET code2_fa = ?, code2_faexpiry = ? WHERE email = ?";
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            if (rowsUpdated > 0) {
                EmailSender.send(email, "Votre code de vérification 2FA est : " + code + ". Ne le partagez avec personne.");
            }
    ...
    public boolean confirmCode2FA(String email, String code) {
        Connection conn = dataBaseHelper.getInstance().getConnection();
        String query = "SELECT code2_fa, code2_faexpiry FROM user WHERE email = ?";
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            if (rs.next()) {
                String storedCode = rs.getString("code2_fa");
                Timestamp expiry = rs.getTimestamp("code2_faexpiry");

                if (storedCode != null && expiry != null) {
                    LocalDateTime now = LocalDateTime.now();
                    if (storedCode.equals(code) && expiry.toLocalDateTime().isAfter(now)) {
                        // Code correct et pas expiré
                        String updateQuery = "UPDATE user SET is2_fa = ?, code2_fa = NULL, code2_faexpiry = NULL WHERE email = ?";
                        try (PreparedStatement stmt2 = conn.prepareStatement(updateQuery)) {
