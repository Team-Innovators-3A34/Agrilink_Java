...
public class PostsService implements IService<Posts> {
    ...
    public List<Map<String, Object>> rechercherPostsWithUserData(int userId) throws SQLException {
        List<Map<String, Object>> postsWithUserData = new ArrayList<>();
        Connection conn = dataBaseHelper.getInstance().getConnection();

        String query = "SELECT p.*, u.nom, u.prenom, u.image as user_image " +
                "FROM posts p " +
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            ...
            while (rs.next()) {
                ...
                post.setUser_id_id(rs.getInt("user_id_id"));
                ...
                // User data
                postData.put("userName", rs.getString("nom") + " " + rs.getString("prenom"));
                postData.put("userImage", rs.getString("user_image"));
